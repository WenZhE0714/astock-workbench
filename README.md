...
| 能力 | 状态 | 说明 |
|---|---|---|
| 实时行情 | 可用 | 腾讯公开 Level-1，股票列表/完整详情原地切换，含真实涨速与资金流 |
...
| 智能市场报告 | 可用 | 后台扫描大盘量能、板块承接、热门龙头与 A/B/C 智能观察池 |
| 个股多维研判 | 可用 | 技术、资金、热门板块、龙虎榜、公告和舆情线索的简洁 AI 报告 |
| 看盘 AI 咨询 | 可用 | `x` 后台咨询当前股票，自动带入实时多维数据并支持内存追问 |
...
`Esc` 返回原看盘页面。扫描综合三大指数未复权日线和量能、沪深京成交额、行业涨跌/主力资金/广度、
...
`~/.local/share/astock-workbench/market-reports/<时间戳>/`，包含 Markdown、快照 JSON 和元数据。
可用 `ASTOCK_CODEX_BIN`、`ASTOCK_CODEX_MODEL`、`ASTOCK_CODEX_PROFILE` 和
`ASTOCK_CODEX_TIMEOUT_SECONDS` 覆盖 Codex 可执行文件、模型、配置档与 30-1800 秒超时。
在自选、榜单、详情或资金雷达中按 `c`，会在后台生成当前股票的多维研判，行情继续刷新；完成后
按 `o` 打开。报告分别给出短线/波段多空理由、当前累计主力与可用的1/3/5分钟变化、关联板块涨跌/
主力/广度/排名、近30日龙虎榜、公告和第三方新闻标题线索，以及来自未复权日K的支撑、压力、
买入观察条件、卖出/减仓观察条件和判断失效位。点位必须沿用MA20/MA60或20日结构高低点，
Codex不得创造数据中没有的价格；新闻和公告标题不能当成已经核验的公司事实。

在自选、榜单、详情或资金雷达中按 `x` 输入问题，例如“现在是否适合买入”“我的成本是 25 元、
三成仓位应该怎么观察”。提交后立即回到行情刷新，状态栏显示 Agent 的数据采集和回答进度；完成后
再按 `x` 打开可滚动回答页，回答页按 `x` 可继续追问同一只股票。切换股票后会开启新对话；历史只
保存在当前进程内存，重启清空。每次问答自动带入当前行情、未复权日 K 技术结构、当日累计资金、
...
`snapshot.json` 和 `metadata.json`。新闻标题来自东方财富搜索，可用 `ASTOCK_STOCK_NEWS_API_URL`
指定兼容代理；它仅属于舆情线索层，正式结论仍以交易所、巨潮或公司公告PDF为准。
