...
同花顺 Quant API(可选) ─> market adapter ──> Quote/Minute/DailyBar ─> terminal UI/strategy
...
- `internal/market`：个股/指数行情、沪深个股涨跌幅/涨速榜、未复权日 K、沪深京历史成交额、个股与全行业主力资金流、行业板块成份股成交额排行、关联板块排行、龙虎榜、公告索引、新闻搜索、券商研报索引和名称解析适配器，不包含策略逻辑。可选同花顺 Quant API 通过 `--source ths` 接入实时行情、高频序列和日期序列，失败时回退现有 HTTP 源；个股榜单直接携带东方财富行业分类；默认日 K 先使用东方财富，自动回退腾讯未复权序列，再回退14天内本地有效缓存；上一交易日成交额使用新浪上证指数、深证综指和北证 50 的 5 分钟精确成交额按日汇总。
...
- `internal/paper`：只读信号驱动的影子账户与 A 股撮合规则，输出操作、持仓、费用、容量和拒绝原因；不实现 `execution.Broker`，因此不能提交真实订单。
...
外部模拟通道优先采用独立适配器接入，而不是把券商 SDK 直接放入研究层。macOS 主程序通过受限 HTTP/gRPC 桥接 Windows 侧 miniQMT/xtquant；桥接层只实现 `execution.Broker` 和账户查询契约，凭证不进入 Web 或信号归档。上线顺序必须是只读账户同步、模拟账户委托、人工确认实盘，三者分别配置且默认关闭。
